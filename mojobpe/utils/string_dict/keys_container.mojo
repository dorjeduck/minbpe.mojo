...
struct KeysContainer[KeyEndType: DType = DType.uint32](Sized):
    ...
    fn add(mut self, key: String):
        ...
        if count >= self.capacity:
            ...
            var keys_end = UnsafePointer[Scalar[KeyEndType]].alloc(
                self.allocated_bytes
            )
    ...
    fn get(self, index: Int) -> StringSlice[StaticConstantOrigin]:
        ...
        return StringSlice[StaticConstantOrigin](
            ptr=self.keys.offset(start), length=length
        )
    ...
    #fn keys_vec(self) -> InlineArray[StringSlice[StaticConstantOrigin], 0]:
    #    var keys = InlineArray[StringSlice[StaticConstantOrigin], 0]()
